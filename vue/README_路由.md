...
## 7. 编程式路由导航
    this.$router.push(location): 相当于点击路由链接(可以返回到当前路由界面)
    this.$router.replace(location): 用新路由替换当前路由(不可以返回到当前路由界面)
    	location为字符串: 可以包含路由路径/params参数/query参数
		location为对象: 对象中可以包含path/name/params/query配置属性
